"""
main.py
DCF Valuation Tool — Streamlit entry point.

Changes from original:
- growth_stats() is cached via @st.cache_data to avoid recomputation on every re-render
- Input validation guards against edge cases (all-negative FCF, inverted ranges)
- Sensitivity tab uses real DCFEngine.sensitivity_analysis() data
- Monte Carlo RNG is created with a user-visible seed for reproducibility option
"""
...
from visualization import (
    plot_distribution, plot_percentiles, plot_waterfall,
    plot_sensitivity, display_summary,
)
...
def growth_stats(vals: tuple) -> dict:
    """
    Calculate mean and std of historical growth rates.
    Cached — only recomputes when the input tuple changes.

    Parameters
    ----------
    vals : tuple of floats (not list, so it's hashable for caching)
    """
    arr = np.array(vals, dtype=float)
    if len(arr) < 2:
        return {'mean': 0.05, 'std': 0.03}
    growth_rates = np.diff(arr) / arr[:-1]
    growth_rates = np.clip(growth_rates, -0.5, 1.0)
    return {
        'mean': float(growth_rates.mean()),
        'std': float(growth_rates.std()) if len(growth_rates) > 1 else 0.03,
    }
...
def validate_inputs(
    ...
    if all(f <= 0 for f in fcfs):
        errors.append("At least one year of Free Cash Flow must be positive.")
...
def main():
    ...
    with st.sidebar:
        ...
        mp = st.number_input(
            "Market Risk Premium", 0.03, 0.12,
            st.session_state.get('mp_calc', 0.065), 0.005, format="%.3f",
        )
        ...
        use_fixed_seed = st.checkbox("Fix random seed", value=False,
                                     help="Enable for reproducible results across runs")
    ...
    with col1:
        ...
        beta = st.number_input(
            "Beta Coefficient", 0.1, 3.0,
            st.session_state.get('fetched_beta', 1.0), 0.01, format="%.3f",
        )
    ...
    with col2:
        ...
        rev_cols = st.columns(n_hist_years)
        for i, col in enumerate(rev_cols):
            yr = curr_yr - i
            revs.append(col.number_input(
                f"{yr}", 0.0,
                value=round(1000.0 * (1.05 ** (n_hist_years - i - 1)), 1),
                format="%.1f", key=f"r{yr}",
            ))

        st.markdown("**Free Cash Flow (millions)**")
        fcf_cols = st.columns(n_hist_years)
        for i, col in enumerate(fcf_cols):
            yr = curr_yr - i
            fcfs.append(col.number_input(
                f"{yr}", -1000.0,
                value=round(100.0 * (1.05 ** (n_hist_years - i - 1)), 1),
                format="%.1f", key=f"f{yr}",
            ))
        ...
        ce   = rf + beta * mp
        we   = 1.0 / (1.0 + d2e)
        wacc = we * ce + (1.0 - we) * cod * (1.0 - tax)

        m_cols = st.columns(4)
        m_cols[0].markdown(metric_card("Avg Revenue Growth", fmt_pct(rev_g['mean']),
                                       ('neutral', f"σ: {fmt_pct(rev_g['std'])}")), unsafe_allow_html=True)
        m_cols[1].markdown(metric_card("Avg FCF Growth", fmt_pct(fcf_g['mean']),
                                       ('neutral', f"σ: {fmt_pct(fcf_g['std'])}")), unsafe_allow_html=True)
        m_cols[2].markdown(metric_card("Avg FCF Margin", fmt_pct(fcf_margin)), unsafe_allow_html=True)
        m_cols[3].markdown(metric_card("WACC (indicative)", fmt_pct(wacc)), unsafe_allow_html=True)
        ...
        if use_range:
            ...
            for i in range(n_years):
                ...
                with c2: min_g = c2.number_input("Min", -0.5, 1.0, max(center - 0.05, -0.1), 0.01, format="%.3f", key=f"ming{i}")
                with c3: max_g = c3.number_input("Max", -0.5, 1.0, min(center + 0.05, 0.3),  0.01, format="%.3f", key=f"maxg{i}")
            ...
            for i, col in enumerate(g_cols):
                g = col.number_input(
                    f"Year {i+1}", -0.5, 1.0,
                    max(base_g * (0.9 ** i), 0.02), 0.01, format="%.3f", key=f"g{i}",
                )
        ...
        if use_range:
            c1, c2 = st.columns(2)
            tg_min = c1.number_input("Min Terminal", 0.0, 0.04, 0.02, 0.005, format="%.3f")
            tg_max = c2.number_input("Max Terminal", 0.0, 0.05, 0.03, 0.005, format="%.3f")
    ...
    if st.button("Run DCF Valuation", type="primary", use_container_width=True):
        ...
        if errors:
            ...
            unc = {
                'fcf_growth_std':      max(fcf_g['std'], 0.03),
            ...
            with st.spinner(f"Running {n_sims:,} simulations…"):
                ...
                # Run sensitivity analysis with real parameter perturbation
    ...
    if st.session_state.valuation_results:
        ...
        with tab2:
            scenarios = {
                'bear':     res['percentiles']['p10'],
                'base':     res['percentiles']['p50'],
                'bull':     res['percentiles']['p90'],
                'expected': res['mean'],
            }
            ...
            scen_rows = [
                {'Scenario': name, 'Fair Value': fmt_curr(scenarios[key], curr),
                 'Upside/Downside': fmt_pct(scenarios[key] / price - 1)}
                for name, key in [
                    ('Bear (P10)',       'bear'),
                    ('Base (P50)',       'base'),
                    ('Bull (P90)',       'bull'),
        ...
        with tab3:
            # Uses real sensitivity data computed by DCFEngine
        ...
        export_data = {
            'summary': {
                'company':            company,
                'current_price':      price,
                'mean_fair_value':    res['mean'],
                'upside_probability': f"{up_prob:.1f}%",
